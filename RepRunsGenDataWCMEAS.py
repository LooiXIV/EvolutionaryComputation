...
algoNames = ["NSGAII", "SPEA2", "DE", "CMEAS", "Nelder-Mead"]
...
for axes, sig in zip(Allaxes, Sigmas):
    ...
    errorCM = np.array([fm.FitnessFuncSO(salCM[seed], y0, t, data, RMSE=True) for seed in salCM.keys()])
    ...
    errorStds = [[np.std(errorNSGAII1), np.std(errorNSGAII2)],
                  ...
                  np.std(errorNM)]
    #print(parmsLB)
    allParamsEsts = [parmsNSGAII, parmsSPEA2, parmsDE, parmsCM, parmsNM]
    algoFig = {}
    ...
    for meanLabs, stdLabs, ax, algoParams in zip(errorMeans, errorStds, axes, allParamsEsts):
        ...
        for params in algoParams:
            ...
            ax.plot(t, sol[:,1], alpha=0.05, color="g")
            ax2.plot(t, sol[:,2], alpha=0.05, color="darkorange")
...
plt.savefig("Figures/RepPlotsGenDataWCMEAS.png", dpi=300)
