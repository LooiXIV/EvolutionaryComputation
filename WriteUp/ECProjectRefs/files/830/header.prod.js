...
$(function(){
    ...
    var displayCookieBanner = function() {
        // Only show if not already dismissed.
        if (document.cookie.indexOf(DISMISSED_COOKIE) == -1 && $('.wrapper.cda').length == 0) {
            $('#cookies-dismiss').on('click', dismiss);
            $('#cookie-banner').removeClass('hidden').show();
        }
    };
    ...
    var dismiss = function(evt) {
        ...
        setCookie(DISMISSED_COOKIE, 'yes');
        $('#cookie-banner').hide();
    };
    ...
    // kickoff everything on document ready
    $(function() {
        displayCookieBanner();
    });
}(jQuery));