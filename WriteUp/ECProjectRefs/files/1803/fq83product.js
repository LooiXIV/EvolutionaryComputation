...
var redirectUri=document.getElementsByName('redirectUri')[0].value;document.getElementsByName('redirectUri')[0].value=encodeURI('/action/resetTfaMethod?redirectUri='+redirectUri);};/*! jQuery UI - v1.10.2 - 2013-03-14
* http://jqueryui.com
* Copyright 2013 jQuery Foundation and other contributors; Licensed MIT */
...
$(document).ready(function(){SearchFacetsUtil.bindFacetHandlers();});var SearchFacetsUtil={bindFacetHandlers:function(){$('.facetHeader').on('click',SearchFacetsUtil.toggleFacetContainer);$('.expand').on('click',SearchFacetsUtil.expandNestedFacets);$('.showMore').on('click',SearchFacetsUtil.showMoreFacetNodesHandler);$('.showLess').on('click',SearchFacetsUtil.showLessFacetNodesHandler);$('.facet-link-container').on('click',SearchFacetsUtil.checkInputStatus);$('.moreFacet').on('click',SearchFacetsUtil.showMoreChildNodes);$('.chosen-select.facets').each(function(){var preventAjaxTrigger=false;if($(this).hasClass("js__preventAjaxTrigger")){preventAjaxTrigger=true;}
var mis=$(this).magicSuggest({hideTrigger:'true',allowFreeEntries:'false',expandOnFocus:true,maxSelection:1,placeholder:$(this).data('placeholder')});if(preventAjaxTrigger){$(mis).on('selectionchange',function(e,m,selection){if(selection!=undefined){window.location.href=selection[0].id;}});}
else{$(mis).on('selectionchange',function(e,m,selection){if(selection!=undefined){SearchUtil.resultAjaxHandler(false,selection[0].id);}});}});},expandNestedFacets:function(){$(this).siblings('.childrenFacets').slideToggle('slow');$(this).toggle();$(this).siblings('.showFacetToggler').toggle();},showMoreChildNodes:function(){},checkInputStatus:function(){},showMoreFacetNodesHandler:function(e){e.preventDefault();$(this).siblings('.hiddenChildrenFacets').slideToggle('slow',function(){$(this).find('input').focus();$(this).find('.dropdown-menu').removeClass('ms-res-ctn-scrollable');$(this).find('.dropdown-menu').scrollintoview({duration:"1000",complete:function(){$(this).find('.dropdown-menu').addClass('ms-res-ctn-scrollable');}});});$(this).parent().find('.hiddenChildrenFacets').find('.showLess').show();$(this).hide();},showLessFacetNodesHandler:function(e){e.preventDefault();$(this).closest('.hiddenChildrenFacets').slideToggle('slow');$(this).closest('.facetContainer').find('.showMore').show();},toggleFacetContainer:function(){$(this).parent().find('.facetContainer').slideToggle("slow");}};//Extension of FigureViewerJs.js

function initViewer(element,e,imageSize) {
    ...
    if (!fvw.length) {
        ...
        if (c.length) {
            ...
            fvw.addClass("overlay-bg");
            c.append(fvw);

            fvw = $(document.createElement("div"));
            fvw.attr('id','figureViewer-wrap');
            fvw.css("display",'none');
            c.append(fvw);
            $(fvw).html(
                '<div id="figureViewer">\
                        <div id="figureViewerNav">\
                            <a title="Close Figure Viewer" href="#" onclick="closeFigure(); return false;" class="close"></a>\
                            <div class="figureNav">\
                                <a class="prev" title="Previous Figure" href="#" class="disabled"></a>\
                                <span id="NavText"><span class="index">1</span> of ' + figures.length + '</span>\
							    <a class="next" title="Next Figure" href="#" class="disabled"></a>\
						    </div>\
					    </div>\
                        <div id="figureCanvas">\
    ...
    if ($(element).attr("data-id")) {
        figureElement = $("#" + $(element).attr("data-id"));
    } else {
...
//
        if(figureElement.attr('id').indexOf('ref-') > -1){
            figureId =   figureElement.attr('id').substring('ref-'.length);
        }else if(figureElement.attr('id').indexOf('extrafiglink-') > -1){
            figureId =   figureElement.attr('id').substring('extrafiglink-'.length)
        }else{
            figureId = figureElement.attr('id')
        }
        while (index < figures.length && figures[index].i != figureId) index++;
    }
    showImage(index,imageSize);
...
function showImage(index,imageSize) {
    var figures = window.figureViewer.figures;
    var figureMeta = figures[index];
    if (!figureMeta) figureMeta = figures[index=0];
    var graphics = figureMeta.g;
    var table = $("#tableBody-" + figureMeta.i);
    var figureElement = $("#"+figureMeta.i);
    var fvw = $('#figureViewer-wrap');
    var html = "";

    if(figureMeta.type=='table-wrap' && table.html()) {
        html = table.html();
    } else {
        /* image and caption */
        for (var i = 0; i < graphics.length; i++) {
            html += '<img alt="Figure" src="' + window.figureViewer.path + '/images/'+imageSize+'/' + ((imageSize == 'medium') ? graphics[i].m : graphics[i].l) +'"/>';
        }
    }
    ...
    // M.Rawajfeh, Bug[LIT-91050] : show figure attributes in the figure viewer
    // NOTE : the code below assume that these attributes are translated to html as <span> element with a css class = NLM_attrib
    var figureAttributesNotPopulatedYet = $(".attributes",fvw).length == 0;
    var showFigureAttributes = ( $("#showFigureAttributes").val() == 'true' );
    if (showFigureAttributes && figureAttributesNotPopulatedYet) {

        var attributesDiv = $(document.createElement( "div" ));
        $(attributesDiv).addClass( "attributes" );

        var attributesItems = $( ".NLM_attrib", figureElement );
        for (var index = 0; index < attributesItems.length; index ++) {
            var attributeSpan = $(attributesItems[index]).clone();
            $(attributesDiv).append( attributeSpan );
            if (index < attributesItems.length -1) {
                // add line break after each attribute if its not the last one
                $(attributesDiv).append( "<br/>" );
            }
        }
        $(".caption",fvw).after( attributesDiv );
    }
    ...
    if (index == 0) {
        ...
        pLink.attr("href", "JavaScript:showImage(" + (index - 1) + ",'" + imageSize + "')");
    }
    ...
    if (index == figures.length) {
        ...
        nLink.attr("href","JavaScript:showImage(" + index + ",'" + imageSize + "')");
    }
    ...
    html = '';
    var imageCaption = $("#viewLargeImageCaption").val();
    var highResClass = "highRes";
    var downloadPPTClass = "downloadPPT";
    if (table && figureMeta.type=='table-wrap')
    {
        $('#figureCanvas',fvw).prepend($(".caption",fvw));
        imageCaption = "View Table Image";
        highResClass = "highResTable";
        downloadPPTClass = "downloadPPTTable";
    } else {
        $('#figureViewer-footer',fvw).before($(".caption",fvw));
    }
    for (var i = 0; i < graphics.length; i++) {
        if (graphics[i].l) {
            if (html) html += '<br />'; // is not first link
            html += '<a class="'+ highResClass + '" target=_blank href="' + window.figureViewer.path + '/images/large/' + graphics[i].l + '"><span>' + imageCaption;
            if (graphics.length > 1) {
                html += ' (' + (i + 1) + ' of ' + graphics.length + ')';
            }
            html += '</span></a>';
        }
    }
    if (html) {
        ...
        ftHtml += '<div>' + html + '<a class="' + downloadPPTClass + '" target=_blank href="/action/downloadFigures?doi=' +  encodeURIComponent(articleDOI) + '&id=' + figureMeta.i + '">Download PowerPoint</a>' + '</div>';
    }
...
function displayWithEffect(viewerId, e, funct) {
    ...
    if (!viewer) return; // It may take some time till document gets ready
        if ($("#figureViewer-wrap").height() && $("#figureViewer-wrap").width()) {
            posY = (($(window).height() - ( ($("#figureViewer-wrap").height() < 590) ? 590 : $("#figureViewer-wrap").height()) ) / 2) + $(document).scrollTop();
            posX = ($(window).width() - (($("#figureViewer-wrap").width() < 850) ? 850 : $("#figureViewer-wrap").width())) / 2;
//            $("#figureViewer-wrap").css("position", "fixed");
        }
        else if (e.clientY && e.clientX) {
            posY = e.clientY + document.body.scrollTop + document.documentElement.scrollTop;
            posX = e.clientX + document.body.width + document.documentElement.width;
        } else {
            posY = $(window).scrollTop() + Math.floor($(window).height() / 2);
            posX = $(window).width() + Math.floor($(window).width() / 4);
        }
        ...
        viewer.css("left", posX + "px");
        viewer.css("top", posY + "px");
        viewer.mouseSelectionFixed = true;
        var refViewer = $("#refViewer-wrap");
        if (refViewer[0] != null)
            refViewer.draggable({cancel: "div#refCanvas"});
        clearRefLinksFromDialog();
        // clicking off the viewer hides the viewer
}


$(document).unbind('mouseup.FigureViewer');
var InTheNews = InTheNews || {};


InTheNews.IndexWidget = (function($){

    function IndexWidget(element){
        this.$element = $(element);
        this.$element.find('h3 a').on('click',this.handleGroupHeaderClick.bind(this));
        var expanded = this.$element.find('.in-the-news-group.expanded')
        if(expanded){
            this.expandedGroup = {link: expanded.find('h3 a'), group: expanded};
        }
    }

    IndexWidget.prototype.expandGroup = function($link,$group){
        $link.attr('aria-expanded','true');
        $group.addClass('expanded');
        this.expandedGroup = {link : $link, group: $group};
    }

    IndexWidget.prototype.collapseGroup = function($link,$group){
        $link.attr('aria-expanded','false');
        $group.removeClass('expanded');
        this.expandedGroup = null;
    }

    IndexWidget.prototype.handleGroupHeaderClick = function(e){
        e.preventDefault();
        var $link = $(e.target);
        var $group =  $link.parents('.in-the-news-group');

        if($group.hasClass('expanded')){
            this.collapseGroup($link,$group);
        }else {
            if(this.expandedGroup){
                this.collapseGroup(this.expandedGroup.link,this.expandedGroup.group )
            }
            this.expandGroup($link, $group);
        }
    }
...
$(document).ready(function(){
    $(".in-the-news-index").each(function (){new InTheNews.IndexWidget(this)});
})
;
$(document).ready(function(){$('.searchResults select ,.tocListWidget .publicationToolSelect').each(function(){jcf.replace($(this));});var MenuDrawerModule=(function(){var appbarElement=$('.app-bar'),darkbgElement=$('.navdrawer-bg'),menuBtn=$('.appbar-icons-container'),pageBodyContainer=$('.pageBody'),mainBody=$('body'),index=0,oldindex=[],touchstarty=0,touchmovey=0,movey=0,quartViewport=parseInt($(window).height()/24);function appBarActions(){var state=menuBtn.attr('aria-expanded')==='false'?true:false;appbarElement.toggleClass('menu-open');pageBodyContainer.toggleClass('menu-open');mainBody.toggleClass('no-scroll');menuBtn.attr('aria-expanded',state);darkbgElement.toggleClass('overlay-fixed');}
return{bindUIActions:function(){$(document).on('click','.appbar-icons-container',function(){appBarActions();});darkbgElement.on("touchstart click",function(e){e.preventDefault();appBarActions();});$(".navdrawer-items .expander").click(function(e){var self=$(this);if(!(self.parent().hasClass("menu-rail"))){if(self.parents().hasClass("menu-rail")){self.parents(".menu-rail").addClass("sub-opened");}
...
index=focusableElements.length-1;}else{index--;focusableElements[index].focus();}}});$('.navdrawer-container').on('touchmove',function(e){e.preventDefault();});}};})();MenuDrawerModule.bindUIActions();$(".ebookSidebarDownloadLinks a").click(function(){var file_type=$(this).attr("data-attribute");var journal=$(this).attr("doi");var journal_info=journal.slice(8);var journal_name=journal_info.slice(0,journal_info.indexOf('.'));var journal_code=journal_info.substr(journal_info.indexOf('.')+1);ga('send','event','Button',file_type,journal_name,journal_code);})});$(document).on("click touchstart",function(e){if($('.search-open').is(':visible')&&!$(e.target).closest('.search-open').length&&!$(e.target).closest('.mini-search').length&&!$('.quickSearchResultPage').length){e.preventDefault();toggleSearch();}
if($(e.target).closest('.mini-search').length){e.preventDefault();toggleSearch();}});var lastScrollTop=0;$(document).on("scroll",function(e){var st=$(this).scrollTop();if(st<lastScrollTop&&$(window).scrollTop()<53&&$('.quickSearchResultPage .search-open').css('display')=='none'){$('.quickSearchWidget .search-open').show();}
lastScrollTop=st;});function toggleSearch(e){if($('.search-open').is(":visible")){$('.search-open').slideUp("linear",function(){$('.search-bg').fadeOut("fast");});$(".mini-search").removeClass("active");}else{$('.search-open').slideDown("linear");$('.search-bg').show();$(".mini-search").addClass("active");}
$('.searchText').first().focus();if(document.hasFocus()&&!$('.search-open').is(":visible")){$('.searchText').first().blur();}
return false;}
function showHideAbs(doi,idToShow,idToHide){$("#"+idToHide).hide();$("#"+idToHide).removeClass("shown");$("#"+idToShow).show();$("#"+idToShow).addClass("shown");showAbstract(doi,idToShow+" .abstractSection");}
function showAbstract(doi_param,div){if($("#"+div).html()==""){var data={ajax:'true',doi:doi_param};jQuery.ajax({url:'/action/showAjaxAbstract',type:'POST',data:data,success:function(data,status){populateAbstract(data,div);}});}}
function populateAbstract(msg,div){$("#"+div).html(msg);}
function expandAbstracts(aForm){if(aForm){var hasMarked,elmts=aForm.elements;for(var i=0;i<elmts.length;i++){if((elmts[i].name=="doi")&&(elmts[i].type=="checkbox")&&(elmts[i].checked)){hasMarked=true;var dois=elmts[i].value;var id=dois.substring(dois.indexOf('/')+1);id=id.replace(/[-/(/)/.]/g,'');id=id.replace(':','');var showAbsId="tr[id = 'showAbs"+id+"']";var absId="tr[id = 'Abs"+id+"']";if($(showAbsId).is(":visible")){$(showAbsId).click();}else{$(absId).click();}}}
if(!hasMarked){alert("Please check at least one article.");}}else{if($("tr[id^=showAbs]").is(":visible")){$("tr[id^=showAbs]").click();}else{$("tr[id^=Abs]").click();}}};/*!
